...
def loadCam(args, id, cam_info, resolution_scale,args1=None):
    ...
    if hasattr(cam_info,"mask_tensor"):
        if cam_info.mask_tensor is not None:
            mask_tensor = cam_info.mask_tensor
    ...
    if args1:
        ...
        if args1.xyz:
            if args1.inverse:
                args1.angle = -args1.angle 
            if args1.xyz == 'x':
                cam_info.T[0] = cam_info.T[0] + args1.angle
            elif args1.xyz == 'y':
                cam_info.T[1] = cam_info.T[1] + args1.angle
            elif args1.xyz == 'z':
                cam_info.T[2] = cam_info.T[2] + args1.angle
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
                  ...
                  mask = mask_tensor if cam_info.mask_tensor is not None else None)

def cameraList_from_camInfos(cam_infos, resolution_scale, args,args1=None):
    camera_list = []

    for id, c in enumerate(cam_infos):
        camera_list.append(loadCam(args, id, c, resolution_scale,args1=args1))
