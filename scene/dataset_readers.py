    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if os.path.exists(mask_path):
            ...
            cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, width=width, height=height, fid=fid, mask_tensor=mask_tensor)
        else:
            cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, width=width, height=height, fid=fid)
        cam_infos.append(cam_info)
    print("camera num", len(cam_infos))
    sys.stdout.write('\n')
    return cam_infos
...
def readCamerasFromNpy(path, npy_file, split, hold_id, num_images):
    ...
    video_paths = sorted(glob(os.path.join(path, 'images/*')))
